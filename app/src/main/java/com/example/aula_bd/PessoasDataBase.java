...
public class PessoasDataBase extends SQLiteOpenHelper{
    ...
    public Pessoas selecionarPessoa (int cod){
        ...
        Cursor cursor = db.query(tb_pessoas,new String[]{c_cod,c_nome,c_tel,c_email},c_cod+"= ?",new String[]{String.valueOf(cod)},null,null, null);
        if(cursor!=null) cursor.moveToFirst();
        else return null;
        Pessoas pessoa1 = new Pessoas(Integer.parseInt(cursor.getString(0)),cursor.getString(1), cursor.getString(2),cursor.getString(3));
        return pessoa1;
    }
    ...
    public void atualizarPessoa (Pessoas pessoa){
        ...
        values.put(c_tel,pessoa.getTel());
        values.put(c_email,pessoa.getEmail());
        db.update(tb_pessoas,values,c_cod + " = ?", new String[]{String.valueOf(pessoa.getCod())});
        db.close();
    }


    public PessoasDataBase(@Nullable Context context) {
        super(context,nomeBd, null, versao);
    }

    public void onCreate(SQLiteDatabase db){
        String query = "CREATE TABLE " + tb_pessoas + "("+ c_cod + " INTEGER PRIMARY KEY, " +c_nome + " TEXT, "+ c_tel + " TEXT, "+c_email+ "TEXT)";
        db.execSQL(query);
